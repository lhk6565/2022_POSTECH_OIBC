...
6. 평가 산식은 아래와 같음
$$ \displaystyle\sum_{d} \displaystyle\sum_{h=1}^{24} \displaystyle\sum_{n\in{10,17}} \left( 0.45 \frac{\left|\frac{U_{d,h}^n + L_{d,h}^n}{2} - G_{d,h}\right|}{C} + 0.45 \frac{U_{d,h}^n - L_{d,h}^n}{C} + 0.1 \frac{G_{d,h}I \left\{ L_{d,h}>G_{d,h} \;or\; U_{d,h}<G_{d,h} \right\}}{C} \right) $$
